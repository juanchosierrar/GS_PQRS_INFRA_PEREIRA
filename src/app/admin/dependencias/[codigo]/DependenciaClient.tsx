...
import { Building2, UserPlus, Loader2, UserCircle, AlertCircle, CheckCircle, TrendingUp, LayoutDashboard, Clock, Mail, MessageCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { format, parseISO, differenceInDays, isPast } from 'date-fns';

export default function DependenciaClient() {
    ...
    const assignMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            const tecnico = allUsers.find(u => u.id === variables.tecnicoId);
            if (tecnico) {
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                <p className="text-xs text-muted-foreground">Asignado a: <span className="font-bold text-primary">{tecnico?.nombre}</span></p>
                                                <div className="flex gap-1">
                                                    {pqr.notificadoEmail && <span title="Email Enviado"><Mail className="h-3 w-3 text-emerald-500" /></span>}
                                                    {pqr.notificadoWhatsapp && <span title="WhatsApp Enviado"><MessageCircle className="h-3 w-3 text-emerald-500" /></span>}
                                                </div>
                                            </div>
                                        </div>
